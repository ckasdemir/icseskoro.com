...
@section('content')
    <div id="content-header">
        <div id="breadcrumb"><a href="#" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Anasayfa</a> <a
                href="#" class="current">Partner Yönetimi</a></div>
        <h1>Partner Yönetimi</h1>
    </div>
                ...
                <div class="widget-box">
                    <div class="widget-title"><span class="icon"><i class="icon-th"></i></span>
                        ...
                        <div style="float: right; margin: 3px 10px 3px 3px; overflow: hidden">
                            <a href="{{route('partners.create')}}" class="btn btn-success"><i class="icon-plus"></i>
                                Yeni
                                Partner Ekle</a>
                        </div>
                    </div>
                            ...
                            @foreach($partners as $item)
                                <tr class="gradeA">
                                    <td style="text-align: center">
                                        @if(!empty($item->logo))
                                            <a href="/uploads/partners/{{$item->logo}}"
                                               target="_blank"
                                               class="tip"
                                    ...
                                    <td style="text-align: center">
                                        @if($item->status == 1)
                                            <a href="{{route('partners.passive', $item->id)}}">
                                                <i class="icon-ok text-success" title="Aktif"></i>
                                            </a>
                                        @else
                                            <a href="{{route('partners.active', $item->id)}}">
                                    ...
                                    <td style="text-align: center">
                                        @if(!$loop->last)
                                            <a href="{{route('partners.down', $item->id)}}"
                                        ...
                                        @if(!$loop->first)
                                            <a href="{{route('partners.up', $item->id)}}"
                                    ...
                                    <td style="text-align: center">
                                        <a href="{{route('partners.edit', $item->id)}}" class="btn btn-info btn-mini"
                                           style="float: left; margin-right: 5px"><i
                                                class="icon icon-edit"></i> Düzenle</a>
                                        {!! Form::model($item,['route'=>['partners.destroy',$item->id],'method'=>'DELETE', 'style'=>'float:left']) !!}
