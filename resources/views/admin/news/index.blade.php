...
@section('content')
    <div id="content-header">
        <div id="breadcrumb"><a href="{{route('admin.index')}}" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Anasayfa</a> <a
                href="#" class="current">Haber Yönetimi</a></div>
                ...
                <div class="widget-box">
                    <div class="widget-title"><span class="icon"><i class="icon-th"></i></span>
                        ...
                        <div style="float: right; margin: 3px 10px 3px 3px; overflow: hidden">
                            <a href="{{route('news.create')}}" class="btn btn-success"><i class="icon-plus"></i> Yeni
                                Haber Ekle</a>
                        </div>
                    </div>
                            ...
                            @foreach($news as $item)
                                <tr class="gradeA">
                                    <td style="text-align: center">
                                        @if(!empty($item->image))
                                            <a href="/uploads/news/{{$item->image}}" target="_blank" class="tip"
                                    ...
                                    <td style="text-align: center">
                                        @if($item->status == 1)
                                            <a href="{{route('news.passive', $item->id)}}">
                                    ...
                                    <td style="text-align: center">
                                        <a href="{{route('news.edit', $item->id)}}" class="btn btn-info btn-mini"
                                           style="float: left; margin-right: 5px"><i
                                                class="icon icon-edit"></i> Düzenle</a>
                                        {!! Form::model($item,['route'=>['news.destroy',$item->id],'method'=>'DELETE', 'style'=>'float:left']) !!}
