                ...
                <div class="widget-box">
                    <div class="widget-title"><span class="icon"><i class="icon-th"></i></span>
                        ...
                        <div style="float: right; margin: 3px 10px 3px 3px; overflow: hidden">
                            <a href="{{route('albums.create')}}" class="btn btn-success"><i
                                ...
                                <tr class="gradeA">
                                    <td style="text-align: center">
                                        @if(!empty($item->image) && file_exists(public_path("/uploads/albums/".$item->folder_name."/".$item->image)))
                                            <a href="/uploads/albums/{{$item->folder_name."/".$item->image}}"
                                               target="_blank"
                                               class="tip"
                                    ...
                                    <td style="text-align: center">
                                        @if($item->status == 1)
                                            <a href="{{route('albums.passive', $item->id)}}">
                                                <i class="icon-ok text-success" title="Aktif"></i>
                                            </a>
                                        @else
                                            <a href="{{route('albums.active', $item->id)}}">
                                                <i class="icon-off text-error" title="Pasif"></i>
                                            </a>
                                        @endif
                                    </td>
                                    <td style="text-align: center">
                                        <a href="{{route('albums.edit', $item->id)}}"
                                           class="btn btn-info btn-mini"
                                           style="float: left; margin-right: 5px"><i
                                                class="icon icon-edit"></i> Düzenle</a>
                                        {!! Form::model($item,['route'=>['albums.destroy',$item->id],'method'=>'DELETE', 'style'=>'float:left']) !!}
